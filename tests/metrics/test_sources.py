...
BACKSTOP_MOVING_THRESHOLD_JUSTIFICATION: Final[str] = (
    ...
    "reasoning for retaining the inherited value rather than presenting the"
    " number as originated in this layer. It is a slow-walking-pace floor "
    "below which a single instantaneous speed sample cannot be reliably "
    "distinguished from GPS noise at a standstill; treating a sample above "
    "it as 'moving' keeps brief stationary noise from being counted as "
    "movement without also discarding genuine slow walking. A pair that "
    "fails the speed test falls through to whether cumulative distance "
    "increased across it -- this is not restricted to a speed-less channel,"
    " so a present-but-low speed reaches the distance check too. (Corrected"
    " 2026-08-23, queue 2026-07-29-moving-threshold-justification-over-"
    "narrows and 2026-07-28-moving-fallback-not-speed-less-only: this "
    "parenthetical previously read 'or a cumulative-distance increase "
    "across the pair, for a speed-less channel', which described a narrower"
    " rule than aggregates.py implements. The identical wording was already"
    " corrected in aggregates.py's own docstring at task 10.1; this copy "
    "was ruled out of that task's boundary and left standing. The code is "
    "what Req 7.1 specifies -- 'session timer, else threshold + distance-"
    "increase fallback', with no speed-less qualifier -- so the record was "
    "the thing that drifted and no computed value moves. Consequence now "
    "stated rather than implied: on an activity with a speed channel and a "
    "noisy distance channel, a standing-still sample whose distance ticks "
    "up is counted as moving.) That reasoning is why fitdocs keeps the "
    "inherited value rather than replacing it: it is a judgment call about "
    "where that noise floor sits, not a value either fitdocs.ai or this "
    "layer read off measured data."
)
