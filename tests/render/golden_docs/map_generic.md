---
title: Workout 2021-09-07 19:46
type: workout
generator: fitdocs
doc_version: 5
date: '2021-09-07'
start_time: '2021-09-07T19:46:40-06:00'
sport: Workout
modality: other
distance_km: 0.0
moving_time: 0:03
avg_hr_bpm: 102
sources:
- fit-archive/41f7b7a3161b321b3ef05b7178f9f241392d94701660499c20d037a0c893cbb6.fit
---

<!-- fitdocs:generated: everything outside the notes/workout/load regions is replaced on regeneration -- see this directory's AGENTS.md -->

# Workout 2021-09-07 19:46

<!-- fitdocs:begin:notes -->
_Your notes go here. This section is preserved when the document is regenerated._
<!-- fitdocs:end:notes -->

## Summary

| Stat | Value |
| --- | --- |
| Distance | 0.00 km |
| Moving time | 0:03 (elapsed 0:03) |
| Avg HR | 102 bpm (max 103 bpm) |

## Map

![Route map](assets/2021-09-07-workout-1946-map.svg)

## Telemetry

**HR** 102 bpm · **TRIMP** 0

![Telemetry hero chart](assets/2021-09-07-workout-1946-hero.svg)

![Heart-rate time-in-zone](assets/2021-09-07-workout-1946-zones.svg)

## Training Load

<!-- fitdocs:begin:load -->
_Training load not computed._
<!-- fitdocs:end:load -->

## Device & Data Quality

**Channel coverage**

| Channel | Coverage |
| --- | --- |
| Heart rate | 100% |
| Distance | 100% |

**Decode errors:** none
