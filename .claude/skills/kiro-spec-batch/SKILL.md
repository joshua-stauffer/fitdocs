...
**Change-protocol preflight**: this skill writes tracked files across many `.kiro/specs/*/`. Before dispatching subagents, confirm the tree is on a branch, not `main`; if it is not, create the worktree first (`.kiro/steering/change-protocol.md`) — subagents inherit the tree they are launched in, so a batch started on `main` puts every spec there. Done means merged back to `main` with validation green, not files written.

**Shared-log preflight**: this is the one skill that manufactures concurrency rather than merely coexisting with it — it can put several specs in flight at once, none of which a peer running `/kiro-impl` would otherwise expect to be unstable. Before Step 1, `cat "$(git rev-parse --git-common-dir)/agent-log"` — this is the only way to learn that a peer already holds a `CLAIM` on a feature this run is about to (re)write; if one is live, drop that feature from this run rather than overwrite work in flight, unless the claiming session itself already closed it (`MERGED`/`RELEASE`) or the maintainer has confirmed that session is gone, in which case log an `OVERRIDE` before this run's own `CLAIM` line for that feature. Claim staleness is defined once, in `.kiro/steering/concurrency.md` § Stale claims — no skill in this repo carries a local heuristic for it. Subagents dispatched below do not read or write the log themselves — this controller session owns both on their behalf, per `.kiro/steering/concurrency.md`. Use `spec-batch` as this session's id for every log line it writes.
...
**Claim granularity is per-wave, not per-run**: claiming every feature in the roadmap up front would hold every one of them against a peer for this run's entire duration, which is worse than not claiming at all on a multi-wave batch. Instead, immediately before dispatching a wave's subagents, append one claim line naming every feature entering that wave:
```bash
LOG="$(git rev-parse --git-common-dir)/agent-log"
printf '%s\t%s\t%s\t%s\n' "$(date -u +%FT%TZ)" "spec-batch" CLAIM "wave <N>: <feature-a> (worktree <path-a>, branch <name-a>), <feature-b> (worktree <path-b>, branch <name-b>), ..." >> "$LOG"
```
...
4. **Free the wave's claim**: append `RELEASE` naming the features that finished (spec files written, approvals set — a peer running `/kiro-impl` can now claim them for implementation) and any that failed (still unowned, but not silently re-claimable without knowing why they failed):
   ```bash
   LOG="$(git rev-parse --git-common-dir)/agent-log"
   printf '%s\t%s\t%s\t%s\n' "$(date -u +%FT%TZ)" "spec-batch" RELEASE "wave <N> done: <feature-a> spec complete, <feature-b> failed (<reason>)" >> "$LOG"
   ```
