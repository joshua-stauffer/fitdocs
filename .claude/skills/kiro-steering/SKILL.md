...
**Change-protocol preflight**: this skill writes tracked files (`.kiro/steering/*.md`), and a steering edit is non-trivial by default — it changes how every later session behaves. Before the first write, confirm the tree is on a branch, not `main`; if it is not, create the worktree first (`.kiro/steering/change-protocol.md`). Done means merged back to `main` with validation green, not files written.
