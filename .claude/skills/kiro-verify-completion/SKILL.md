...
- when the task changed tests, **discrimination evidence**: for each new or changed assertion, the mutation that was applied and the test that went red. Green proves the assertions ran, not that they could fail — return `NOT_VERIFIED` when a completeness claim rests on a passing suite alone. Contract and anti-patterns: `.kiro/steering/change-protocol.md` § Fixture Discrimination
- no claim of discrimination that rests only on prose — a comment or report line saying a mutation was caught is not evidence the mutation was run
...
| “The new tests pass, so the behavior is pinned” | A passing assertion may be unable to fail. The mutation is the evidence, not the green. |
| “The test comment says the mutation was caught” | Prose is not evidence. Only a mutation that was actually run is. |
