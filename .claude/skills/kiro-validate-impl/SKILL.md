...
**Shared-log preflight**: `cat "$(git rev-parse --git-common-dir)/agent-log"` before gathering context. This skill can run while a peer is mid-implementation in a different worktree, and a failure this run reports may be caused by a peer's landed work rather than by the feature under validation — exactly the read point `.kiro/steering/concurrency.md` names ("when something fails in a way your own diff does not explain"). Note any `MERGED` since this feature's tasks were last touched, and any `WARN` naming a module this feature also depends on. This skill does not hold a claim on the feature — it validates, it does not implement — so it has nothing to `CLAIM`; if a mechanical or integration failure below turns out to be explained by a peer's `WARN` rather than a local defect, name that peer's log line in the report instead of filing it as `LOCAL`.
...
**Either decision — surfaced follow-ups**:
Validation is where cross-task and cross-spec problems become visible, and
most of them are not this feature's to fix. Record them in `.kiro/queue/` via
the `kiro-queue-add` skill: findings classified `UPSTREAM` (with
`UPSTREAM_SPEC` as the `area`), coverage gaps left unaddressed, architecture
drift accepted rather than corrected, and dependent specs the report says
should be revalidated. Name each queued `id` in your response. A `GO` that
silently swallows an upstream finding is how it gets rediscovered three specs
later.
