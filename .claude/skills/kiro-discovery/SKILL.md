...
**Change-protocol preflight**: this skill writes tracked files (`brief.md`, `.kiro/steering/roadmap.md`). Before the first write, confirm the tree is on a branch, not `main`; if it is not, create the worktree first (`.kiro/steering/change-protocol.md`). Done means merged back to `main` with validation green, not files written.
