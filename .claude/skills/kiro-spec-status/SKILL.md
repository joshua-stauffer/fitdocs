...
- **Queue directive**: if the arguments contain `[queue: <path>]`, ignore it —
  kiro-spec-status is a read-only report with nothing to scope a run to. Do
  not read that file, and do not let the directive text be parsed as a
  feature name — the feature is the first token only.
- Extract the feature name from the first whitespace-separated token of
  `$ARGUMENTS`; ignore any remainder (e.g. a queue resume command's
  `[queue: ...] <intent>` text appended after the feature name). Call this
  `{feature}`.
- Read `.kiro/specs/{feature}/spec.json` for metadata and phase status
- Read `.kiro/specs/{feature}/brief.md` if it exists
- Read existing files: `requirements.md`, `design.md`, `tasks.md` (if they exist)
- Check `.kiro/specs/{feature}/` directory for available files
...
**Spec Not Found**:
- **Message**: "No spec found for `{feature}`. Check available specs in `.kiro/specs/`"
