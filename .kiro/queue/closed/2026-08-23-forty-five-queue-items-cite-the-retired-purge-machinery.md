...
**Closed `done` 2026-08-23 — the triage this item asked for was run.**

All 45 items were read by their `## What` section, not by their path, as this
item required. Outcome:

- **45 closed as `done`** — the subject was the retired one-shot tooling
  (`scripts/purge/*`, `tests/purge/*`) or a precondition on a purge task that
  has since run. Retirement is the resolution; there is no future run.
- **3 closed as `done` on verified evidence** — the Req 11.4 notice/trademark
  items, confirmed satisfied by `git grep` returning zero tracked hits for the
  reserved-rights phrase and the trademark sentence.
- **11 dropped** — internal staleness in the completed spec's own `design.md` /
  `tasks.md` / `requirements.md`. Archival; nothing consults them.
- **7 re-pointed and kept open** — the subject survived in a retained guard
  (`tests/_forbidden_strings.py`, `tests/test_forbidden_strings.py`,
  `tests/_content_oracle.py`, `tests/test_content_oracle.py`) or in
  `docs/reference/history-rewrites.md`. Each carries a `## Triage note`.
- **~20 kept untouched** — items whose subject was always a surviving artifact
  (the retained guards, the durable provenance record, the two sdist release
  gates); their locators were never stale.

The item's own done criterion is met: no open item's `resume_command` or
`context:` names a path that does not exist. The two meta-items that name
`scripts/purge/` as their *subject* are both now closed.

The four items citing destroyed scratch artifacts were handled in the same
pass: `2026-08-01-scratch-artifacts-not-durable` closed `done` (task 9.4
destroyed them under a maintainer ruling, superseding the request), and
`2026-08-23-provenance-record-cites-seven-destroyed-scratch-files` was kept
open — it is against the durable record, which still ships.

Queue went 199 open -> 143 open, 71 closed -> 127 closed.
