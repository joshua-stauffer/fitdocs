...
- [ ] 1.4 Declare the release policy data *(rewritten by Amendment 2)*
...
- [ ] 2. Release tooling
- [ ] 2.1 (P) Build the release artifact builder *(rewritten by Amendment 2: no profiles)*
...
- [ ] 2.2 Build the artifact conformance checker *(rewritten by Amendment 2: link-member rule added; not parallel with 2.1 — both write into the new `scripts/` package and both add to `tests/test_release_artifacts.py`)*
...
- [ ] 2.3 Add the encumbered-content gate to the checker *(rewritten by Amendment 2: reuses the purge's guard cores, fails closed)*
...
- [ ] 3.2 Validate the one artifact set end to end *(rewritten by Amendment 2; formerly "Validate the unencumbered release end to end")*
