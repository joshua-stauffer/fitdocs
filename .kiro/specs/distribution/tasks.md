...
- [x] 5.5 Write the contribution documentation
...
- [x] 5.6 Write the release procedure document
...
- [x] 5.7 Add the documentation entry point and the preserved-guarantee test
...
- [ ] 6.1 Establish the publication prerequisites
  ...
  - _Blocked: requires maintainer action outside the repository (2026-09-19, impl-distribution run). Checklist for Josh: (1) on pypi.org and test.pypi.org, add a **pending** Trusted Publisher for `fitdocs` naming owner `joshua-stauffer`, repository `fitdocs`, workflow filename `release.yml`, environment `pypi` (public) / `testpypi` (rehearsal) -- no placeholder upload; (2) in the GitHub repo create environments `testpypi` (no reviewers) and `pypi` (required reviewer: you); (3) add repository secret `FITDOCS_FORBIDDEN_STRINGS_CONTENT` = the contents of your out-of-repository match-data file. The workflow files (6.2/6.3/6.4) are authored against exactly those names; their live-run observables are manual-verify until this lands._

- [x] 6.2 Add the continuous integration workflow
...
- [x] 6.3 Add the tag-triggered gate and verification chain
...
- [x] 6.4 Add the publication and post-publication chain
...
- [x] 7.2 Validate the preserved guarantees
...
- [ ] 7.3 Rehearse a full release
  ...
  - _Blocked: depends on 6.1 (maintainer-only: pending Trusted Publisher on pypi.org and test.pypi.org, GitHub environments `testpypi`/`pypi`, repository secret `FITDOCS_FORBIDDEN_STRINGS_CONTENT`) and on a real tag push through `release.yml`; nothing inside the repository can rehearse publication. Everything the rehearsal exercises short of the indexes is already automated and verified: the gate chain executes in CI (run 35423230711+ shows tests/lint/types/build green and the fail-closed stop at the secret step), the stale-changelog trip is pinned by `tests/test_release_artifacts.py` (pairwise version gate), and installation from a wheel into a fresh environment is pinned by `tests/test_packaging.py`. Runbook for Josh once 6.1 is done: docs/releasing.md steps 1-10 as ONE run -- the release heading regex and `tests/test_changelog.py` accept only `X.Y.Z`, so there is no `rc` tag, and `skip-existing: false` means a version spent on TestPyPI by a separate rehearsal would fail the real run's `publish-testpypi`; the rehearsal IS steps 8 and 10a (TestPyPI publish + install from test.pypi.org) inside the same run, and the `pypi` environment approval is the go/no-go between them. Trip test: before that, push a tag whose CHANGELOG entry is deliberately stale and confirm the run stops at `version` with nothing published, then delete the tag.
...
- 5.5: `docs/compatibility.md` has no per-version notes -- per-release contract notices live in CHANGELOG entries (Req 4.3); point calculator authors there. CONTRIBUTING.md:41-43 says CI runs the three gate commands on push/PR -- task 6.2 must make that literally true (`uv run pytest`; `uv run ruff check . && uv run ruff format --check .`; `uv run mypy`). Reuse `tests/test_docs_guarantees.py::_code_block_after` (marker-anchored) for plugins.md surface lists; a fence-pairing regex over a section breaks the moment a fence gains a language tag.
- 5.6: the release literal lives in THREE places bumped together (pyproject + both SKILL.md metadata.version). ONLY an unset FITDOCS_FORBIDDEN_STRINGS yields gate_not_run (exit 1); set-but-unusable is the core's hard error (exit 2). Invoke an isolated install by path (`"$UV_TOOL_BIN_DIR/fitdocs"`) -- a bare `fitdocs` resolves to the global shim. `--extra-index-url` OUTRANKS `--index-url` under uv's first-index strategy and both are deprecated: use `--index https://test.pypi.org/simple/`. No manual `uv publish`: publish is the workflow's OIDC jobs (`publish-testpypi` auto, `publish-pypi` behind environment approval). Job names 6.3/6.4 MUST use: gates, version, build, check, verify-artifact, publish-testpypi, publish-pypi, verify-published (`tests/test_releasing_docs.py` pins the step->job table). RELEASE BLOCKERS (queued, high): `tests/test_version_identity.py`'s docs/plugins.md allowlist and `tests/test_changelog.py`'s two pre-release pins red at the first version cut.
- 5.7: `tests/test_docs_guarantees.py::_shipped_documentation_text` now covers README + top-level `docs/*.md` ONLY (excludes `docs/reference/`, whose provenance record carried a meta-mention that satisfied the never-delete guard). The 1.9 form test has TWO detectors: `](...)` link targets and bare `docs/*.md` paths (with well-formed project-URL links masked first -- the raw regex false-positives on backticked link labels); a project URL pinned to a non-`main` ref would be flagged. Preserved-statement pins are single verbatim polarity-bearing phrases, not token proximity.
- 6.2: `.github/workflows/ci.yml` (job `gates`). MANUAL VERIFY once 6.1 lands: push a branch and confirm the run is green with the secret set, and that a policy-breaking change fails with the checker's listing. ACTION PINS: `astral-sh/setup-uv` publishes NO floating major tag since v8 -- `@v10` is a 404; pin `@v10.1.0` (sha bec219d24cd3e171d82865faccec33120bb574f4) or the SHA; a pin-format test must accept vX.Y[.Z] or a 40-hex SHA and reject `@main`. Fork PRs fail at the secret-write step by design (secrets are empty on fork PRs) -- CONTRIBUTING should say so (queued). Tests of a workflow must EXECUTE the parsed `run:` bodies (`bash -eo pipefail`), not hand-copied argv; an assertion on a step NAME is an ever-present token.
- 6.3: `.github/workflows/release.yml` carries the five gate jobs; 6.4 APPENDS `publish-testpypi`, `publish-pypi`, `verify-published` to the SAME file (comments mark where) and must relax `tests/test_release_workflow.py`'s "publishes nothing yet" and job-set assertions deliberately. Workflow-test lessons: pin the inter-job artifact hand-off (upload path == download paths == consumer paths), assert no job-level `permissions` on gate jobs, and EXECUTE the secret-write guard body -- token presence (`exit 1`, `gate_not_run`) is an ever-present token. `actions/upload-artifact@v4.4+` drops hidden files (uv build's `.gitignore`) -- harmless. `uv tool install --from dist/fitdocs-*.whl fitdocs` installs from the wheel (receipt proves it); the automation omits `--offline` (no runner cache). MANUAL VERIFY once 6.1 lands: push a `v*` tag and watch the chain.
- 6.4: `release.yml` is complete (eight jobs). Publishing action pinned `pypa/gh-action-pypi-publish@v1.14.2` (immutable: true, sha a892a5a61159132606e93a2fa6f4358831b04d26); `id-token: write` ONLY on the two publish jobs (job-level permissions REPLACE the top-level block; same-run download-artifact needs no extra scope); approval lives on the `pypi` ENVIRONMENT, not in YAML. Execution tests that install from the network must be made hermetic: substitute `--no-cache`→`--offline` and `sleep 30`→`sleep 0` once each, fake HOME/XDG_BIN_HOME/XDG_DATA_HOME under tmp_path, dead proxies, `UV_CACHE_DIR` = the real warm cache; a falsity test must assert the INSTALL succeeded before trusting exit 1. Queued: 6.3's verify-artifact execution test still uses the non-hermetic executor; `uv build` in fixtures re-fetches hatchling's index when uv's cache goes stale (7.2 offline observable). MANUAL VERIFY once 6.1 lands: tag push -> TestPyPI auto-publish -> approve `pypi` -> verify-published green.
- 7.1: Installed-tool E2E lives in `tests/test_packaging.py` (fixture `_installed_tool_from_wheel` yields `(exe, env, tool_dir, cwd_dir)`). Help-listing assertions must be ROW-ANCHORED (`^\W*<cmd>\s{2,}`, re.M) -- a bare substring `"load" in stdout` is satisfied by `--help` prose. Hermetic env must `pop` FITDOCS_DATA/FITDOCS_FORBIDDEN_STRINGS and pass an explicit tmp `cwd` -- the developer's data root leaked in and produced a false failure. CI's cache is `uv sync`-only: `uv tool install --offline` / `uv pip install --offline` FAIL against it, so offline fixtures need a try-offline -> warm-online-once (proxies stripped) -> retry-offline shape; verify with `UV_CACHE_DIR=<scratch> UV_PROJECT_ENVIRONMENT=<scratch>/venv uv sync` then run the tests from that venv. A `main()`-raises mutation does NOT pin `--help`/`--version` (Click's eager handling short-circuits); an import-time `version(DIST_NAME)` read does.
