...
- The **owned-path set** — `workouts/`, `workouts/assets/`, `fit-archive/`, `.cache/`, and `.fitdocs/` (tool-owned state under the data root, e.g. inbox's quarantine record) — and the **configured-location clause**: a location the user's settings name as a place fitdocs writes (inbox's intake directory and its optional `processed/` destination) is granted create/write rights by that configuration; such locations are named in `fitdocs.toml`, not fixed by the contract. Fitdocs writes nowhere outside the owned set plus the configured locations (2.1, 2.10, 7.5). _(Amendment 2, 2026-09-11)_ `history/` and `history/assets/` are also fitdocs-owned; naming and writing them is `load-history`'s, not this spec's. _(Amendment 3, 2026-09-15)_ `blocks/` is also fitdocs-owned, holding the two further document types training-blocks names; naming and writing it is training-blocks' own, not this spec's. The plan-source directory training-blocks reads is a configured *read* location, not an owned path: naming it in `fitdocs.toml` grants fitdocs no create or write right there, unlike the configured-location clause above, which grants write.
...
**Amendment (2026-09-15), landed by `training-blocks`:** the data root gains two further document types, the training-block page and the planned-workout page, written to a new owned location (`blocks/`) this module does not name and does not write into. The type constants, both pages' frontmatter schema, and their own format versions (`block_version`, `planned_version`) are declared in `fitdocs.plans.page`, not here -- like the history page, neither type is ever routed through `is_workout_document`, `MANAGED_KEYS`, `DOC_VERSION`, or any other reader this module defines, and no reader here is extended to recognise either. The reason both types live outside this leaf rather than as new values published from it is the same one Amendment (2026-09-11, `load-history`) gives for the history page: `src/fitdocs/contract.py` stays the single most heavily pinned leaf, and a spec that adds a document type owns that type's vocabulary in its own package. This spec's Requirement 2 criteria 2.12 and 2.13 record only that the plan-source location is user-owned and read-only, and that the rendered location is fitdocs-owned and holds two further document types -- not what either type's schema is; Requirement 3 criterion 3.10 records what the rendered location's own in-tree declaration states about those documents, without restating either type or schema.
