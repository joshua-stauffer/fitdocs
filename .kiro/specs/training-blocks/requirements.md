...
4. The fitdocs CLI shall accept, within one amendment, any number of changes of these kinds: a change to the named fields of an existing planned workout, the addition of a planned workout, the removal of a planned workout by id, and a change to a mesocycle's target load or focus.
...
4. The fitdocs CLI shall produce byte-identical pages for an unchanged source across repeated runs, across different calendar days, and across supported platforms, and shall read no clock: the only dates it uses are the source's own.
...
8. The fitdocs CLI shall not run the plan pass as part of the sync, regeneration, load or history passes.
