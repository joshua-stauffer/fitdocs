...
- [ ] 5.4 Register the pass in the confinement guard and pin the package's surface and boundary
  ...
  - Add the package boundary test: parse every module under the package and check
    ...
    no module names a clock function in code or in a docstring; assert the
    reference-directory string appears nowhere in the package
  - Pin the package's published surface name for name, asserting each name is the
    ...
    contract-consumer guard binding the shared read, the workout-document test,
    the date reader, the effort-tag reader and the load keys by identity, so the
    guard's existing structural assertions apply to it unchanged
