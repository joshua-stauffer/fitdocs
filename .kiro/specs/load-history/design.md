...
**Responsibilities & Constraints**
- Peer of `load_load_settings`, `load_tile_settings`, `load_inbox_settings`,
...
- `BLOCKED_PRESETS` declares the Performance Management Chart pair (42 d / 7 d)
  ...
  `CitedConstant` carries 42.0 or 7.0 as a time constant.
...
- **Colour carries provenance.** `palette.py` is not a bag of hex strings:
  ...
  `tests/render/charts/test_palette.py:114` reconverts every entry's oklch source
...
- **What the declaration refresh implies, stated plainly**: `ensure_declarations`
  (`declaration.py:398`) iterates *all* of `DECLARED_DIRS`, so a history run may
...
- One new command named `history` — distinct from `performance-benchmarks`'
  own command, which the sibling spec names `benchmarks derive`. Neither spec
...
- **Import closure** (modelled on `tests/load/threshold/test_boundary.py`): every
  ...
  by name.
- **Clock scan**: no module under the package names `date.today`, `datetime.now`,
  `datetime.utcnow`, `time.time` or `time.monotonic`, in code or in a docstring
  example.
...
- **Consumer registration**: `fitdocs.history.documents` joins
  `CONVERTED_MODULES` and `CONTRACT_BINDINGS` binding `read_frontmatter`,
  `is_workout_document`, `document_date`, `effort_tag` and `LOAD_KEYS` by
  identity. The existing structural assertions then apply to it unchanged: no
